...
from portable_paths import output_path
...
class ExerciseManager:
    def __init__(self, replay, log_filename, style, resting_hr, max_hr, user_id):
        ...
        self.max_hr = max_hr
        self.last_valid_heart_rate = resting_hr
        self.last_heart_rate_time = None
        self.heart_rate_stale_after_sec = 10
        self.warned_missing_heart_rate = False
        self.warned_stale_heart_rate = False
        ...
        fh = logging.FileHandler(output_path('exercise_coach', 'logs', log_filename))
        ...
        if style == 5: #adaptive
            ...
            adaptive_env['PEPPER_COACH_PARTICIPANT_ID'] = str(user_id)
            self.process = subprocess.Popen([sys.executable, '-u', rp.get_path('pepper_exercise_coach') + '/scripts/adaptive_controller.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True, env=adaptive_env)
        else:
            self.adaptive = False
        self.init_pubs_subs()
    ...
    def heart_rate_callback(self, hr_message):
        self.last_valid_heart_rate = hr_message.data
        self.last_heart_rate_time = rospy.Time.now()
        self.warned_missing_heart_rate = False
        self.warned_stale_heart_rate = False
        if self.curr_set != '':
            self.set_data_dict[self.curr_set]['all_heart_rates'].append(hr_message.data)

    def calculate_hrr(self, heart_rate):
        denominator = float(self.max_hr - self.resting_hr)
        if denominator <= 0:
            self.logger.warning('Invalid HRR range: max_hr {} resting_hr {}'.format(self.max_hr, self.resting_hr))
            return 0.0
        return (heart_rate - self.resting_hr) / denominator

    def get_current_heart_rate(self):
        source = 'live'
        if len(self.set_data_dict[self.curr_set]['all_heart_rates']) > 0:
            heart_rate = self.set_data_dict[self.curr_set]['all_heart_rates'][-1]
            self.last_valid_heart_rate = heart_rate
        elif self.last_valid_heart_rate is not None:
            heart_rate = self.last_valid_heart_rate
            source = 'fallback'
            if not self.warned_missing_heart_rate:
                self.logger.warning('No heart-rate samples for {}; using fallback heart rate {}'.format(self.curr_set, heart_rate))
                self.warned_missing_heart_rate = True
        else:
            heart_rate = self.resting_hr
            source = 'resting_fallback'
            if not self.warned_missing_heart_rate:
                self.logger.warning('No heart-rate data available; using resting heart rate {}'.format(heart_rate))
                self.warned_missing_heart_rate = True

        if self.last_heart_rate_time is not None:
            age = (rospy.Time.now() - self.last_heart_rate_time).to_sec()
            if age > self.heart_rate_stale_after_sec:
                source = 'stale_{}'.format(source)
                if not self.warned_stale_heart_rate:
                    self.logger.warning('Heart-rate data is stale by {:.1f}s; continuing with last heart rate {}'.format(age, heart_rate))
                    self.warned_stale_heart_rate = True

        return heart_rate, self.calculate_hrr(heart_rate), source

    def get_current_hrr(self):
        if self.curr_set != '' and len(self.set_data_dict[self.curr_set]['hrr']) > 0:
            return self.set_data_dict[self.curr_set]['heart_rates'][-1], self.set_data_dict[self.curr_set]['hrr'][-1]
        heart_rate, hrr, source = self.get_current_heart_rate()
        self.logger.warning('Using {} heart-rate context for feedback: HR {}, HRR {}'.format(source, heart_rate, hrr))
        return heart_rate, hrr
    ...
    def get_pepper_message(self, c):
        ...
        for ci in c:
            ...
            # key2 = 'low'
            curr_heart_rate, curr_hrr = self.get_current_hrr()
            if curr_hrr < 0.2:
                key2 = 'low'
            elif curr_hrr < 0.4:
                key2 = 'moderate'
        ...
        if len(m) > 0:
            ...
            for option in m:
                if option in self.set_data_dict[self.curr_set]['message_log']:
                    counts.append(self.set_data_dict[self.curr_set]['message_log'].count(option))
                else:
                    counts.append(0)
    ...
    def react(self, feedback):
        ...
        if self.adaptive:
            ...
            try:
                ...
                self.logger.warning('Adaptive controller returned invalid action "{}"; keeping robot style {}'.format(result, self.robot_style))
    ...
    def pose_callback(self, angle_message):
        ...
        #Save heart rate and hrr. If the sensor drops out, keep the session running
        # with the last known/resting heart rate and log the fallback source.
        curr_heart_rate, hrr, heart_rate_source = self.get_current_heart_rate()
        self.set_data_dict[self.curr_set]['heart_rates'].append(curr_heart_rate)
        self.set_data_dict[self.curr_set]['hrr'].append(hrr)
        self.set_data_dict[self.curr_set]['heart_rate_sources'].append(heart_rate_source)
    ...
    def start_new_set(self, exercise_name, set_num, tot_sets):
        ...
        #add heart-rate source labels to dictionary
        self.set_data_dict[self.curr_set]['heart_rate_sources'] = []
